...
Due to similarities between Maven and Gradle, the simplest way to start is to create a Dockerfile and build upon the Maven slave image. Here is the content of the https://github.com/openshift-evangelists/summit17-cicd-lab/lab-10/blob/master/Dockerfile[Dockerfile on GitHub] for building the Gradle slave image:
